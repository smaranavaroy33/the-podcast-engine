...
def parse_script_json(script_text: str) -> list:
    """
    Parse the script JSON from the LLM output robustly.
    
    Args:
        script_text: Raw script text (may include ```json fences or conversational filler)
    ...
    if start_idx != -1 and end_idx != -1 and end_idx >= start_idx:
        ...
        try:
            ...
            pass # Fall back to basic parsing if this fails
            
    # Fallback to basic fence stripping if we couldn't find a valid array bracket pair
    if text.startswith("```"):
        lines = text.split("\n")
...
def main():
    ...
    if has_any_data or st.session_state.current_stage > 0:
        tab1, tab2, tab4 = st.tabs(["🔍 Research", "📋 Summary", "🎧 Podcast"])
